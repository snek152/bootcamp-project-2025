...
function renderBlogs(items: Blog[], containerId = "blog-container") {
  const container = document.getElementById(containerId);
  if (!container) {
    console.warn(`renderBlogs: container '#${containerId}' not found`);
    return;
  }
  ...
  items.forEach((blog) => {
    ...
    a.href = `/blogs/${blog.slug}`; // point to a blog route (adjust as needed)
